...
function useHistoryStateNavigation(base) {
    ...
    function changeLocation(to, state, replace) {
        ...
        try {
            ...
            vue.warn('[vue-router]: Error with push/replace State', err);
...
function useLink(props) {
    ...
    const href = vue.computed(() => router.createHref(route.value));
    ...
    const isExactActive = vue.computed(() => activeRecordIndex.value === currentRoute.value.matched.length - 1 &&
...
function useView(options) {
    ...
    const matchedRoute = vue.computed(() => vue.unref(options.route).matched[depth]);
    const ViewComponent = vue.computed(() => matchedRoute.value && matchedRoute.value.components[vue.unref(options.name)]);
    const propsData = vue.computed(() => {
        ...
        if (!props)
            return {};
        const route = vue.unref(options.route);
        if (props === true)
            return route.params;
    ...
    function onVnodeMounted() {
        ...
        matchedRoute.value.instances[vue.unref(options.name)] = viewRef.value;
        // TODO: trigger beforeRouteEnter hooks
    }
    return (attrs) => {
        return ViewComponent.value
            ? vue.h(ViewComponent.value, {
                ...propsData.value,
                ...attrs,
                onVnodeMounted,
                ref: viewRef,
...
const View = vue.defineComponent({
    ...
    setup(props, { attrs }) {
        const route = vue.inject(routeLocationKey);
        const renderView = useView({ route, name: vue.toRefs(props).name });
...
function createRouter({ history, routes, scrollBehavior, parseQuery: parseQuery$1 = parseQuery, stringifyQuery: stringifyQuery$1 = stringifyQuery, }) {
    ...
    const currentRoute = vue.ref(START_LOCATION_NORMALIZED);
...
function applyRouterPlugin(app, router) {
    ...
    for (let key in START_LOCATION_NORMALIZED) {
        ...
        reactiveRoute[key] = vue.computed(() => router.currentRoute.value[key]);
    }
    // TODO: merge strats?
...
//# sourceMappingURL=vue-router.cjs.prod.js.map
